    ...
    <Fragment>
      <ul className='mt-3 grid grid-cols-2 gap-1 md:gap-5 sm:gap-6 sm:grid-cols-2 lg:grid-cols-4'>
        {exerciseList.map((exercise, index) => (
          ...
          >
            <div className='flex-shrink-0 flex items-center justify-center w-10 md:w-16 bg-gray-700 text-white text-2xl md:text-3xl leading-5 font-medium rounded-l-md'>
              {index + 1}
            </div>
            <div className='flex-1 flex items-center justify-between text-left border-t border-r border-b border-gray-200 bg-white rounded-r-md truncate md:p-2'>
              <div className='flex-1 pl-1 md:px-4 py-1 text-sm leading-5 truncate '>
                <p className='text-gray-900 whitespace-normal font-medium hover:text-gray-600 transition ease-in-out duration-300 md:mb-2 text-lg md:text-xl leading-tight'>
                  {exercise.name}
                  {exercise.bodyweight && (
